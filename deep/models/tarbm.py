...
class TARBM(object):
    ...
    def __init__(self, input_=None, input_history=None, n_visible=49,
        ...
        """
        TARBM constructor. Defines the parameters of the model along with
    ...
    def free_energy(self, v_sample, v_history):
        ...
        on the history '''
        if self.delay > 0:
            wx_b, _ = self.propup(v_sample, v_history)
            ax_b = self.vbias
        else:
    ...
    def propup(self, vis=None, v_history=None, static=False, delay=None,
               past_sample=False):
    ...
    def sample_h_given_v(self, v0_sample, v_history, past_sample=False):
    ...
    def propdown(self, hid, v_history, static=False):
    ...
    def gibbs_vhv(self, v0_sample, v_history):
        ...
        pre_sigmoid_h1, h1_mean, h1_sample = self.sample_h_given_v(v0_sample,
                                                v_history, past_sample=False)
        #h1_sample = Print('hid_samples')(h1_sample)
        v1_mean, _ = self.sample_v_given_h(h1_sample, v_history)

        return [pre_sigmoid_h1, h1_mean, h1_sample, v1_mean, v1_mean]

    def get_cost_updates(self, delay=0, k=1, persistent=False,
                         static=True, all=False, all_with_W=False):
        """
    ...
    def get_cost_updates_AE(self, delay, corruption,
        ...
        reconstruction = self.propdown(curr_hid, None, static=True)
        if self.binary:
    ...
    #        #TODO: this could probably be faster as a tensordot, but we don't have tensordot for gpu yet
        ...
        if sparse_weight > 0:
            sparse_penalty = T.sum(sparsity * T.log(sparsity/curr_hidd.mean(axis=0))
                            + (1-sparsity) *
                            T.log((1-sparsity) / (1 - curr_hidd.mean(axis=0))))  
...
#
    ...
    def generate(self, orig_data, orig_history, n_samples, n_gibbs=30):
        ...
        # Try sampling from the past values as well. binary not real value
        ...
        #vis_samples = Print('vis_samples')(vis_samples)
